var POReceiptShortcutV7 = (function() {
	...
	*/
	...
	function getTroubleshootingInfo(status, lineFailureDetail) {
		...
		var withDetail = function(lead) {
			return [lead, lineFailureDetail].filter(Boolean).join("\n");
		};
		switch (status) {
			case "10": return "Warehouse transaction message was created but has not been validated yet.";
			case "15": return "Header validation failed. Check MHS850 for the header error.";
			case "20": return "Header validated, but package processing did not start. Check MHS850 for the message details.";
			case "25": return "Package validation failed. Check MHS851 for the package error.";
			case "30": return "Package validated, but line processing did not complete. Check MHS851 for the package details.";
			case "35": return withDetail("Line validation failed. Check MHS851 for the failing line.");
			case "40": return withDetail("Lines validated, but downstream processing did not finish. Check MHS851 for the failing line.");
			case "45": return withDetail("Business validation failed during receipt processing.");
			case "92": return "The transaction ran in test mode, so no inventory update was performed.";
			case "99": return "The warehouse transaction message is archived.";
			default: return "Warehouse transaction ended in status " + status + ". Check MHS850/MHS851 for details.";
		}
	}
	function formatErrorCode(errorCode, errorMessage) {
	...
	*/
	function extractErrorMessage(error, operation) {
		...
		return (error.errorMessage || error.message || fallback) + getTechnicalDetails(error);
	}
	/**
	...
	*/
	...
	var TRANSIENT_ERROR_CODES = ["WPU0901", "M3LOCK"];
