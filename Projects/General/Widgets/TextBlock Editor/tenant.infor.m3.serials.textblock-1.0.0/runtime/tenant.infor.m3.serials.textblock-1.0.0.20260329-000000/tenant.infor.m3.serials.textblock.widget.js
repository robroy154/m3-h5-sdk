	...
	`,
			...
			let SettingsComponent = class SettingsComponent {
			    ...
			    ngAfterViewInit() {
			        ...
			        this.$compactMode = document.querySelector(`#compactMode${this.instanceId}`);
			...
			let TextBlockService = class TextBlockService {
			    ...
			    deleteTextBlockLine(textBlock, lineNumber) {
			        const normalizedLineNo = String(lineNumber ?? "").trim();
			        const remainingLines = (textBlock.textLines ?? []).filter((line) => {
			...
			let NewTextblockComponent = class NewTextblockComponent extends WidgetBase {
			    ...
			    createForm() {
			        const formConfig = {
			            TXVR: [this.textVersion?.TXVR ?? "Serials"],
			            TX40: [this.textVersion?.TX40 ?? "Additional Serials", Validators.required],
			    ...
			    onSaveText(textBlockValues) {
			        ...
			        this.#textBlockService
			            ...
			            next: (resp) => {
			                this.setBusy(false);
			                const result = normalizedValues.TXID || normalizedValues.TXVR || null;
			                if (resp.length) {
			                    this.showToast({
			                        title: "Success",
			    ...
			    get textBlockHeaderId() {
			        return "Serials";
			    }
			    get textBlockHeaderDescription() {
			        return "Additional Serials";
			    }
			};
			__decorate([
			    Input(),
			...
			let WidgetComponent = class WidgetComponent extends WidgetBase {
			    ...
			    onDeleteTextLine(textBlock, line, event) {
			        event.stopPropagation();
			        ...
			        this.openDeleteMessage(messageSettings).then((deleteConfirmed) => {
			            if (!deleteConfirmed)
			                return;
			            this.setBusy(true);
			            this.subscriptions.add(this.#textBlockService.deleteTextBlockLine(textBlock, line?.LINO).subscribe({
			                next: () => {
			                    this.setBusy(false);
			                    this.setTextBlocks();
			                },
