var AddLotControlColumn = class {
    ...
    public static Init(scriptArgs: IScriptArgs): void {
        try {
            ...
            scriptArgs.log.Error(
                'AddLotControlColumn initialization failed: ' + (error.message || error)
            );
        }
    }
    ...
    private run(): void {
        ...
        if (!list) {
            this.log.Warning(
                this.scriptName + ': No grid found on this panel — script aborted'
            );
            return;
        }
    ...
    private formatLotControl(value: string | null): string {
        switch (value) {
            ...
            case '2':
                return '2 - Serial Number Required';
            case '3':
                return '3 - Lot Number Required';
            default:
                return value || 'N/A';
        }
    }
    ...
    private attachEvents(): void {
        this.unsubscribeEvents = this.controller.RequestCompleted.On((e: RequestEventArgs) => {
            try {
                ...
                this.log.Error(
                    this.scriptName + ': Event handler error: ' + (err.message || err)
                );
            }
        });
    }
