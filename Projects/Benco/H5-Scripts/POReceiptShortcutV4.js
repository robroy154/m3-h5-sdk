...
var POReceiptShortcutV4 = class {
    ...
    try {
      ...
      for (const rec of lines) {
        ...
        if (!this.isNonMaterial) {
          ...
          line.record.BREM = `FirstLoc: ${this.WHSL}`;
        }
      ...
      if (transactionStatus !== '90') {
        ...
        const statusLines = [
          ...
          ...(resolvedPackNumber !== undefined
            ? [`Package no: ${resolvedPackNumber}`]
            : []),
          ...(statusResult.item.TRSL
            ? [`Lowest line status: ${statusResult.item.TRSL}`]
            : []),
          ...(statusResult.item.TRSH
            ? [`Highest line status: ${statusResult.item.TRSH}`]
            : []),
        ];
  ...
  num(value) {
    ...
    return (
      Number.parseFloat((value || '0').toString().replace(/[^\d.-]/g, '')) || 0
    );
  }
