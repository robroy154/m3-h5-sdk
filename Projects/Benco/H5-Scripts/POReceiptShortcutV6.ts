...
/*──────────────────────────────────────────────────────────────────────────*/
var POReceiptShortcutV6 = class {

    // ── H5 screen field values (populated in validateFields) ─────────────────
    private PUNO: string;
    private SUNO: string;
    private WHLO: string;
    private PNLI: string;
    ...
    private ITNO: string;
    private OEND: string;
    // ── PO line data (populated in fetchInitialData) ──────────────────────────────
    private PUPR: string;
    private CPPR: string;
    private RORC: string;
    private RORN: string;
    ...
    private GETY: string;
    private PUUN: string;
    // ── PO header data (populated in fetchInitialData) ──────────────────────────────
    private PUDT: string;
    private CUCD: string;
    private FACI: string;
    // ── Item data (populated in fetchInitialData) ──────────────────────────────
    private INDI: string;
    private TPCD: string;
    private EXPD: string;
    // ── Other M3 fields ────────────────────────────────────────────────────
    ...
    private CUNO: string;
    private MSGN: string;
    private isWmsWhs: boolean;
    // ── Progress UI state ──────────────────────────────────────────────────
    private steps: string[];
    private idx: number;
    private progressContent: JQuery | null;
    private progressDialog: any | null;
    // ── Equipment tracking ─────────────────────────────────────────────────
    private createdEquipment: any[];
    // ── Receipt summary (populated after receipt completes) ────────────────
    private receiptSummary: any;
    // ── Item purchase date and free fields ────────────────────────────────
    private FRE3: string;
    private RGDT: string;
    ...
    private dialogInstanceIdCounter: number;
    private mi: any; // MIService (v2+) or MIService.Current instance (v1) — TODO: tighten type
    private ctrl: IInstanceController;
    private log: IScriptLog;
    private userContext: any;
    private company: string;
    private division: string;
    private maxSerialInputLength: number;
    private derivedSerialLength: number;
    private derivedSerialPrefix: string;
    private hashSuffixLength: number;
    private epochSeed: string | null;
  ...
  static Init(args) {
    ...
    try {
      const self = new POReceiptShortcutV6(args);
      ...
      if (args?.log) {
        args.log.Error(
          `POReceiptShortcutV6 initialization failed: ${error.message || error}`
        );
      }
  ...
  async run() {
    try {
      ...
      await this.resolveCustomerOrder();
      await this.advance('Fetch data');
      ...
      if (this.INDI === '2') {
        ...
        const { lot, expi } = await this.promptLot() as any; // User interaction - NO busy
  ...
  async validateFields() {
    ...
    // Note: WHSL validation is deferred to fetchInitialData() after item type determination
    const missingFields = [
      !this.PUNO && 'PUNO',
      !this.PNLI && 'PNLI',
      !this.PNLS && 'PNLS',
  ...
  async fetchInitialData() {
    // All four calls are independent — fire them simultaneously to minimize round-trips.
    // Only the conditional OIS100MI call (resolveCustomerOrder) depends on results here.
    const poLineRequest = new MIRequest();
    ...
    try {
      const [pol, whs, itm, rem] = await Promise.all([
      ...
      this.log.Error(`fetchInitialData failed: ${error.message || error}`);
      throw error;
    }
  }

  async resolveCustomerOrder() {
    ...
    try {
      ...
      this.log.Error(`resolveCustomerOrder failed: ${error.message || error}`);
      throw error;
    }
  }
  ...
  promptSerials() {
    return new Promise((resolve, reject) => {
      ...
      const handleSaveSerials = (model?: any) => {
  ...
  promptLot() {
    return new Promise((resolve, reject) => {
      ...
      dialogContent.showValidationError = function (message) {
        ...
        setTimeout(() => (errorDiv as any).fadeOut(removeErrorDiv), 5000);
      };
  ...
  handleLotSaveAction(dialogContent, lotNumberId?: any, expirationDateRequired?: any, validateLotInputs?: any) {
