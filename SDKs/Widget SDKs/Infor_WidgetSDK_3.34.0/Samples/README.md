...
npm config set InforWidgetSDK:server_port 8888

# The server will now bind to port 8888
npm run server

# Undo the above configuration
npm config rm InforWidgetSDK:server_port
```
...
npm config set InforWidgetSDK:proxy_remote_host example.com

# The proxy will forward requests to 'example.com'
npm run proxy

# Undo the above configuration
npm config rm InforWidgetSDK:proxy_remote_host
```
