...
@Component({
	template: ` <div #contentTranslationView>
		<soho-listview>
...
export class ContentTranslationComponent {
	@ViewChild("contentTranslationView", { read: ViewContainerRef, static: true })
	...
	constructor(
		...
		this.items =
			widgetContext.getSettings().get<ListItem[]>(this.itemKey) || [];

		const lang = widgetContext.getLanguage();

		widgetInstance.actions[0].execute = () => this.onAdd();
		widgetInstance.actions[0].text = lang.add;
	}
	...
	private openDialog(item?: ListItem): void {
		...
		dialog
			.apply((component) => {
				...
				component.instanceId = this.widgetContext.getWidgetInstanceId();
			})
			.open();
	}
}
