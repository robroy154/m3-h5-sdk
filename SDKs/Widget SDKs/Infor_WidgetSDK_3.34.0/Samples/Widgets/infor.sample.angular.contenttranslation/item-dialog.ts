...
@Component({
	template: ` <div #editItemView style="max-width: 390px">
		<div>
			<div class="field">
				<label class="required" for="sample-ct-edt-ttl">{{ lang.title }}</label>
				<input
					[id]="'sample-ct-edt-ttl' + instanceId"
					name="sample-ct-edt-ttl"
					type="text"
					[(ngModel)]="item.title"
					[maxlength]="maxTitle"
			...
			<div class="field">
				<label class="required" for="sample-ct-edt-desc">{{
					lang.description
				}}</label>
				<input
					[id]="'sample-ct-edt-desc' + instanceId"
					type="text"
		...
		<div *ngIf="isTranslation" class="field">
			<button class="btn-secondary" (click)="onTranslations()">
				{{ lang.translations }}
			</button>
		</div>
...
export class EditItemComponent implements OnInit {
	@ViewChild("editItemView", { read: ViewContainerRef, static: true })
