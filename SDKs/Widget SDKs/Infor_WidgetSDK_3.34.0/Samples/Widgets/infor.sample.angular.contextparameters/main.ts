...
@Component({
	template: ` <span class="label">
			Copy and execute below code in the Browser developer tools console to mock
			a drillback to Homepages.
		</span>
		<span class="label">
			To open another page from the drillback, find the page id in My Pages by
			right-clicking the card header.
		</span>
		<span class="label">
...
@Component({
	template: `
		<div #widgetView class="lm-text-align-c lm-padding-xl">
			<p>Parameter one:</p>
			<p class="param-value" [ngClass]="paramValue1 ? 'success' : 'error'">
				{{ paramValue1 || "No value found for URL parameter 'param1'" }}
			</p>
			<p>Parameter two:</p>
			<p class="param-value" [ngClass]="paramValue2 ? 'success' : 'error'">
...
export class ContextParametersComponent implements OnInit {
	@ViewChild("widgetView", { read: ViewContainerRef, static: true })
	...
	constructor(
		...
		widgetInstance.actions[0].execute = () => this.openInformationDialog();
	}
