	...
	template: `
		<ul class="lm-padding-sm">
			@for (event of events; track $index) {
				<li>
					<ids-text class="time">T+{{ event.time }}ms</ids-text>
					<ids-text class="title">[{{ event.title }}]</ids-text>
					<ids-text class="message">{{ event.message }}</ids-text>
				</li>
			}
		</ul>
	`,
		...
		AfterContentChecked
{
	...
	private asyncOperationInterval: ReturnType<typeof setInterval>;
	private counter = 0;
	...
	constructor() {
		...
		instance.bannerBackgroundChanged = (color) =>
			this.bannerBackgroundChanged(color);
	...
	isConfigured(): boolean {
		const configured = this.widgetContext
			.getSettings()
			.get<boolean>("configured");
	...
	bannerBackgroundChanged(newColor: string) {
		this.log(
			"bannerBackgroundChanged",
			"Banner Background Changed to " + newColor,
		);
	}
	...
	private setWidgetAction() {
		this.widgetInstance.actions[0].execute = () => this.clearLog();
	}
}
